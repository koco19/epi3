...
#' for the second round of visit and calculates the variance associated with the estimation of the proportion of ppl.
...
margins_age_sex <- read.csv(here_koco_data("R2/muc_age_structure_KoCo_study_pop.csv"), stringsAsFactors = F)
...
KoCo_BLab$Age2 <- trunc((as.Date(KoCo_BLab$Q6_f1_Geburtsdatum_DDMONYYYY) %--% as.Date(KoCo_BLab$date_R2))/years(1))
...
KoCo_BLab <- KoCo_BLab[setdiff(names(KoCo_BLab), c("Q6_f1_Geburtsdatum_DDMONYYYY", "date_R2", "Age2"))]
...
###
# Consider the positive from round 1 as respondents
...
KoCo_BLab <- merge(KoCo_BLab, as.data.frame(table(KoCo_BLab$hh_id[!is.na(KoCo_BLab$R2_Result_imp)])), by.x = "hh_id", by.y = "Var1", all.x=T)
...
KoCo_BLab <- KoCo_BLab[!is.na(KoCo_BLab$R2_Result_imp), ]
...
# Numbers can be found in Data/R2/KoCo19_202012.xlsx
n_1_house <- 449561
n_2_house <- 204907+2948
n_multi_house <- 828672+5870 - n_1_house - n_2_house

# Selecting the margins for household with or without children in Munich
# Numbers can be found in Data/R2/KoCo19_202012.xlsx
n_house_0_child <- 682708+4509
n_house_1plus_child <- 828672+5870 - n_house_0_child
...
info_ind_nr <- by(info_ind, info_ind$hh_id, function(x){
  ...
  pos <- sum(x[, "R1_Result"] == "Positive", na.rm = TRUE)
...
freq(x = KoCo_BLab$R2_Result_imp, plot=F)

freq(x = KoCo_BLab$R2_Result_imp, w = KoCo_BLab$w_ind_cal, plot=F)
...
KoCo_BLab$DBS_results <- ifelse(KoCo_BLab$R2_Result_imp == "Positive", 1, 0)
...
group <- split(KoCo_BLab, KoCo_BLab$R1_Result)
...
# Combine datasets
KoCo_BLab_group <- do.call(dplyr::bind_rows, KoCo_BLab_group)
...
colnames(data_res) <- paste0("ge_dbs_", 0:length(group))
...
w_p <- sapply(group, function(z){
  # z <- group[[2]]
  freq(z[, "R2_Result_imp"], w = z[, "w_ind_cal"], plot = F)[1:2, 2]
})

w_p <- c(All = freq(x = KoCo_BLab$R2_Result_imp, w = KoCo_BLab$w_ind_cal, plot=F)[2, 2],
...
res_all <- data.frame(Adjust = rep(c("unadjusted", "adjusted"), each = 3),
