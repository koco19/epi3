...
#' for the fifth round of visit and calculates the variance associated with the estimation of the proportion of ppl.
...
run <- function(seed){
  ...
  margins_age_sex <- read.csv(here_koco_data("R5/muc_age_structure_KoCo_study_pop.csv"), stringsAsFactors = F)
  ...
  KoCo_BLab$Age2 <- trunc((as.Date(KoCo_BLab$Q6_f1_Geburtsdatum_DDMONYYYY) %--% as.Date(KoCo_BLab$date_R5))/years(1))
  ...
  KoCo_BLab <- KoCo_BLab[setdiff(names(KoCo_BLab), c("Q6_f1_Geburtsdatum_DDMONYYYY", "date_R5", "Age2"))]
  
  
  #############################
  # Define sample for the fifth round
  #############################
  ...
  KoCo_BLab <- merge(KoCo_BLab, as.data.frame(table(KoCo_BLab$hh_id[!is.na(KoCo_BLab$R5_Result_imp)])), by.x = "hh_id", by.y = "Var1", all.x=T)
  ...
  KoCo_BLab <- KoCo_BLab[!is.na(KoCo_BLab$R5_Result_imp), ]
  ...
  # Numbers can be found in Data/R5/KoCo_19_Tabelle_31_46_2021_11_30.xlsx
  country_germany_unknow <- 885981
  ...
  # Numbers can be found in Data/R5/KoCo_19_2021_11_30.xlsx
  n_1_house <- 447688
  n_2_house <- 206098+2934
  n_multi_house <- 828330+5830 - n_1_house - n_2_house
  
  # Selecting the margins for household with or without children in Munich
  # Numbers cn_nefan be found in Data/R5/KoCo_19_2021_11_30.xlsx
  n_house_0_child <- 681340+4489
  n_house_1plus_child <- 828330+5830 - n_house_0_child
  ...
  KoCo_BLab$hh_vax <- paste(KoCo_BLab$hh_id, KoCo_BLab$R5_Vaccination, sep = "_")
  ...
  info_ind_nr <- by(info_ind, info_ind$hh_id, function(x){
    ...
    pos <- sum(x[, "R1_Result"] == "Positive" | x[, "R2_Result"] == "Positive" |
                 x[, "R3_Result"] == "Positive" | x[, "R4_Result"] == "Positive", na.rm = TRUE)
  ...
  g_logit <- calib(data_house, d = d_house, totals, method="logit",
                   bounds=c(0.3,7.5), max_iter = 2000)  
  ...
  freq(x = KoCo_BLab$R5_Result_imp, plot=F)

  
  freq(x = KoCo_BLab$R5_Vaccination, plot=F)

  
  freq(x = KoCo_BLab$R5_Result_S1_imp, plot=F)
  
  
  freq(x = KoCo_BLab$R5_Result_imp, w = KoCo_BLab$w_ind_cal, plot=F)

  
  freq(x = KoCo_BLab$R5_Vaccination, w = KoCo_BLab$w_ind_cal, plot=F)

  
  freq(x = KoCo_BLab$R5_Result_S1_imp, w = KoCo_BLab$w_ind_cal, plot=F)

  
  
  KoCo_BLab$R5_S1_N <- ifelse(KoCo_BLab$R5_Result_imp == "Positive" | KoCo_BLab$R5_Result_S1_imp == "Positive",
                              "Positive", "Negative")
  
  freq(x = KoCo_BLab$R5_S1_N, plot=F)

  
  
  freq(x = KoCo_BLab$R5_S1_N, w = KoCo_BLab$w_ind_cal, plot=F)

  
  by(KoCo_BLab[KoCo_BLab$R4_Result_imp == "Negative", ], 
     KoCo_BLab[KoCo_BLab$R4_Result_imp == "Negative", "R4_Vaccination"],
     function(x){
       freq(x[, "R5_Result_imp"], plot = F)
     })
  ...
  by(KoCo_BLab[KoCo_BLab$R4_Result_imp == "Negative", ], 
     KoCo_BLab[KoCo_BLab$R4_Result_imp == "Negative", "R4_Vaccination"],
     function(x){
       freq(x[, "R5_Result_imp"], w = x[, "w_ind_cal"], plot = F)
     })

  
  
  by(KoCo_BLab[KoCo_BLab$R4_Result_imp == "Negative", ], 
     KoCo_BLab[KoCo_BLab$R4_Result_imp == "Negative", "R4_Result_S1_imp"],
     function(x){
       freq(x[, "R5_Result_imp"], plot = F)
     })
  ...
  KoCo_BLab$DBS_results <- ifelse(KoCo_BLab$R5_Result_imp == "Positive", 1, 0)
  ...
  KoCo_BLab$S1_results <- ifelse(KoCo_BLab$R5_Result_S1_imp == "Positive", 1, 0)
  ...
  KoCo_BLab$S1_N_results <- ifelse(KoCo_BLab$R5_S1_N == "Positive", 1, 0)
  ...
  group1 <- split(KoCo_BLab, KoCo_BLab$R4_Result_new)
  
  ### For the group of negative, we split between the vaccinated and non vaccinated
  group1.1 <- split(group1$Negative, group1$Negative$R4_Result_S1_imp)
  
  ### Linearization for the two global groups: vaccinated or not
  group2 <- split(KoCo_BLab, KoCo_BLab$R5_Vaccination)
  
  ### Linearization for the two groups: S1 positive or not
  group3 <- split(KoCo_BLab, KoCo_BLab$R5_Result_S1_imp)
  ...
  filt <- KoCo_BLab_group$R4_Result_new == "Negative"
  
  ### Pop of non vaccinated
  
  cond_nv <- filt & KoCo_BLab_group$R4_Result_S1_imp == "Negative"
  ...
  cond_v <- filt & KoCo_BLab_group$R4_Result_S1_imp == "Positive"
  ...
  w_p <- sapply(group, function(z){
    freq(z[, "R5_Result_imp"], w = z[, "w_ind_cal"], plot = F)[1:2, 2]
  })
  
  w_p <- c(N_pos = freq(x = KoCo_BLab$R5_Result_imp, w = KoCo_BLab$w_ind_cal, plot=F)[2, 2],
           S_pos = freq(x = KoCo_BLab$R5_Result_S1_imp, w = KoCo_BLab$w_ind_cal, plot=F)[2, 2],
           S_N_pos = freq(x = KoCo_BLab$R5_S1_N, w = KoCo_BLab_group$w_ind_cal, plot=F)[2, 2],
