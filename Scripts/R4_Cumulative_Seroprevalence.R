...
#' for the fourth round of visit and calculates the variance associated with the estimation of the proportion of ppl.
...
run <- function(seed){
   ...
   margins_age_sex <- read.csv(here_koco_data("R4/muc_age_structure_KoCo_study_pop.csv"), stringsAsFactors = F)
   ...
   KoCo_BLab$Age2 <- trunc((as.Date(KoCo_BLab$Q6_f1_Geburtsdatum_DDMONYYYY) %--% as.Date(KoCo_BLab$date_R4))/years(1))
   ...
   KoCo_BLab <- KoCo_BLab[setdiff(names(KoCo_BLab), c("Q6_f1_Geburtsdatum_DDMONYYYY", "date_R4", "Age2"))]
   
   
   #############################
   # Define sample for the fourth round
   #############################
   ...
   KoCo_BLab <- merge(KoCo_BLab, as.data.frame(table(KoCo_BLab$hh_id[!is.na(KoCo_BLab$R4_Result_imp)])), by.x = "hh_id", by.y = "Var1", all.x=T)
   ...
   KoCo_BLab <- KoCo_BLab[!is.na(KoCo_BLab$R4_Result_imp), ]
   ...
   # Numbers can be found in Data/R4/KoCo_19_Blatt_31_2021_08_31.xlsx
   country_germany_unknow <- 887271
   ...
   # Numbers can be found in Data/R4/KoCo_2021_08_31.xlsx
   n_1_house <- 449829
   n_2_house <- 204870+2892
   n_multi_house <- 829187+5752 - n_1_house - n_2_house
   
   # Selecting the margins for household with or without children in Munich
   # Numbers cn_nefan be found in Data/R4/KoCo_2021_08_31.xlsx
   n_house_0_child <- 682579+4422
   n_house_1plus_child <- 829187+5752 - n_house_0_child
   ...
   ###
   # Age/Sex structure, country of origin , vaccination
   ###
   ...
   info_ind_nr <- by(info_ind, info_ind$hh_id, function(x){
      ...
      pos <- sum(x[, "R1_Result"] == "Positive" | x[, "R2_Result"] == "Positive" |
                    x[, "R3_Result"] == "Positive", na.rm = TRUE)
   ...
   g_logit <- calib(data_house, d = d_house, totals, method="logit",
                    bounds=c(0.3,5.4), max_iter = 2000)  
   ...
   freq(x = KoCo_BLab$R4_Result_imp, plot=F)
   
   
   freq(x = KoCo_BLab$R4_Vaccination, plot=F)
   
   
   freq(x = KoCo_BLab$R4_Result_S1_imp, plot=F)
   
   
   freq(x = KoCo_BLab$R4_Result_imp, w = KoCo_BLab$w_ind_cal, plot=F)
   
   
   freq(x = KoCo_BLab$R4_Vaccination, w = KoCo_BLab$w_ind_cal, plot=F)
   
   
   freq(x = KoCo_BLab$R4_Result_S1_imp, w = KoCo_BLab$w_ind_cal, plot=F)
   
   
   KoCo_BLab$R4_S1_N <- ifelse(KoCo_BLab$R4_Result_imp == "Positive" | KoCo_BLab$R4_Result_S1_imp == "Positive",
                               "Positive", "Negative")
   
   freq(x = KoCo_BLab$R4_S1_N, plot=F)
   
   
   
   freq(x = KoCo_BLab$R4_S1_N, w = KoCo_BLab$w_ind_cal, plot=F)
   
   
   by(KoCo_BLab[KoCo_BLab$R3_Result_imp == "Negative", ], 
      KoCo_BLab[KoCo_BLab$R3_Result_imp == "Negative", "R3_Vaccination"],
      function(x){
         freq(x[, "R4_Result_imp"], plot = F)
      })
   ...
   by(KoCo_BLab[KoCo_BLab$R3_Result_imp == "Negative", ], 
      KoCo_BLab[KoCo_BLab$R3_Result_imp == "Negative", "R3_Vaccination"],
      function(x){
         freq(x[, "R4_Result_imp"], w = x[, "w_ind_cal"], plot = F)
      })
   
   
   by(KoCo_BLab[KoCo_BLab$R3_Result_imp == "Negative", ], 
      KoCo_BLab[KoCo_BLab$R3_Result_imp == "Negative", "R3_Result_S1_imp"],
      function(x){
         freq(x[, "R4_Result_imp"], plot = F)
      })
   ...
   KoCo_BLab$DBS_results <- ifelse(KoCo_BLab$R4_Result_imp == "Positive", 1, 0)
   ...
   KoCo_BLab$S1_results <- ifelse(KoCo_BLab$R4_Result_S1_imp == "Positive", 1, 0)
   ...
   KoCo_BLab$S1_N_results <- ifelse(KoCo_BLab$R4_S1_N == "Positive", 1, 0)
   ...
   group1 <- split(KoCo_BLab, KoCo_BLab$R3_Result_new)
   
   ### For the group of negative, we split between the vaccinated and non vaccinated
   group1.1 <- split(group1$Negative, group1$Negative$R3_Result_S1_imp)
   
   ### Linearization for the two global groups: vaccinated or not
   group2 <- split(KoCo_BLab, KoCo_BLab$R4_Vaccination)
   
   ### Linearization for the two groups: S1 positive or not
   group3 <- split(KoCo_BLab, KoCo_BLab$R4_Result_S1_imp)
   
   group <- do.call(c, list(group1, group1.1, group2, group3))
   
   # Weighted proportions for DBS in each group:
   # - Negative in the past: proportion of positive in the fourth round
   # - Positive in the past: Proportions of positive AND negative in the fourth round
   # We just calculate the proportion p of those who remain positive (sero-persistence) 
   # and calculate the sero-remission by 1-p
   # - Among the group of negative in the past: proportion of positive among the vaccinated and non vaccinated
   # - Non vaccinated: proportion of positive in the fourth round
   # - Vaccinated: Proportions of positive
   # - S1 negative: proportion of positive in the fourth round
   ...
   p_dbs <- c(Negative_Positive = p_dbs[1], Positive_Positive = p_dbs[2],
              Negative_Positive_Non_vax = p_dbs[3], Negative_Positive_Vax = p_dbs[4],
              Non_vax = p_dbs[5], Vax = p_dbs[6], S_neg = p_dbs[7], S_pos = p_dbs[8])
   ...
   KoCo_BLab_group1.1 <- do.call(dplyr::bind_rows, KoCo_BLab_group[3:4])
   
   KoCo_BLab_group2 <- do.call(dplyr::bind_rows, KoCo_BLab_group[5:6])
   
   KoCo_BLab_group3 <- do.call(dplyr::bind_rows, KoCo_BLab_group[7:8])
   
   KoCo_BLab_group <- merge(KoCo_BLab_group1, KoCo_BLab_group1.1[, c("ind_id", "dbs_u_k_5", "dbs_u_k_6")], all.x = TRUE)
   
   KoCo_BLab_group <- merge(KoCo_BLab_group, KoCo_BLab_group2[, c("ind_id", "dbs_u_k_7", "dbs_u_k_8")])
   
   KoCo_BLab_group <- merge(KoCo_BLab_group, KoCo_BLab_group3[, c("ind_id", "dbs_u_k_9", "dbs_u_k_10")])
   
   ###
   # Linearization for the ratio of incidence between vaccinated and non vaccinated
   ###
   
   filt <- KoCo_BLab_group$R3_Result_new == "Negative"
   
   ### Pop of non vaccinated
   
   cond_nv <- filt & KoCo_BLab_group$R3_Result_S1_imp == "Negative"
   
   # target variable
   y_nv <- KoCo_BLab_group$DBS_results
   y_nv[!cond_nv] <- 0
   
   # total infections
   ty_nv <- sum(y_nv * KoCo_BLab_group$w_ind_cal)
   
   # total persons
   n_nv <- sum(KoCo_BLab_group$w_ind_cal[cond_nv])
   
   ### Pop of vaccinated
   
   cond_v <- filt & KoCo_BLab_group$R3_Result_S1_imp == "Positive"
   
   # target variable
   y_v <- KoCo_BLab_group$DBS_results
   y_v[!cond_v] <- 0
   
   # total infections
   ty_v <- sum(y_v * KoCo_BLab_group$w_ind_cal)
   
   # total persons
   n_v <- sum(KoCo_BLab_group$w_ind_cal[cond_v])
   
   KoCo_BLab_group$dbs_u_k_11 <- (n_v * y_nv - ty_nv / n_nv * n_v * cond_nv - ty_nv * n_v / ty_v * y_v + ty_nv * cond_v) / (n_nv * ty_v)
   
   
   ### Change NAs to zero
   KoCo_BLab_group[, paste0("dbs_u_k_", 0:(length(group)+3))][is.na(KoCo_BLab_group[, paste0("dbs_u_k_", 0:(length(group)+3))])] <- 0
   ...
   split_hh <- split(KoCo_BLab_group[, c("hh_id", "w_ind", paste0("dbs_u_k_", 0:(length(group)+3)))], KoCo_BLab_group$hh_id)
   
   hh_u_k <- sapply(split_hh, function(z){
      res <- sapply(0:(length(group)+3), function(x){
   ...
   res.lm <- lapply(0:(length(group)+3), function(x){
   ...
   colnames(data_res) <- paste0("ge_dbs_", 0:(length(group)+3))
   ...
   V2_dbs <- sapply(data_res_split, function(x){
      ...
      V2_dbs <- sapply(0:(length(group)+3), function(y){
   ...
   w_p <- sapply(group, function(z){
      freq(z[, "R4_Result_imp"], w = z[, "w_ind_cal"], plot = F)[1:2, 2]
   })
   
   w_p <- c(N_pos = freq(x = KoCo_BLab$R4_Result_imp, w = KoCo_BLab$w_ind_cal, plot=F)[2, 2],
            S_pos = freq(x = KoCo_BLab$R4_Result_S1_imp, w = KoCo_BLab$w_ind_cal, plot=F)[2, 2],
            S_N_pos = freq(x = KoCo_BLab$R4_S1_N, w = KoCo_BLab_group$w_ind_cal, plot=F)[2, 2],
            Negative_Positive = w_p[2], Positive_Positive = w_p[4],
            Negative_Positive_Non_vax = w_p[6], Negative_Positive_Vax = w_p[8],
            Non_vax = w_p[10], Vax = w_p[12],
            S1_neg = w_p[14], S1_pos = w_p[16],
            Ratio = (ty_nv / n_nv) / (ty_v / n_v))
   ...
   # Negative_Positive_Non_vax: sero-incidence among the non vaccinated persons
   # Negative_Positive_Vax: sero-incidence among the vaccinated persons
   # S1_neg: % of ppl infected by Corona among the S1 neg
   # S1_pos: % of ppl infected by Corona among the S1 pos
   # Ratio: ratio of Negative_Positive_Non_vax / Negative_Positive_Vax
   ...
   w_ub["Ratio"] <- w_p["Ratio"] + qnorm(0.975) * sqrt(V[12])
   
   
   # Lower bound CI
   w_lb <- w_p - qnorm(0.975) * sqrt(V) * 100
   w_lb["Ratio"] <- w_p["Ratio"] - qnorm(0.975) * sqrt(V[12])
   ...
   res_adj <- res_w[rownames(res_w) != "Ratio", ]
   
   
   res_adj <- (res_adj/100 + spec_sens_classifier["Roche N pan-Ig optimized cut-off", "Specificity"] - 1)/(spec_sens_classifier["Roche N pan-Ig optimized cut-off", "Sensitivity"] + spec_sens_classifier["Roche N pan-Ig optimized cut-off", "Specificity"] - 1)*100
   
   ###
   # Save results
   ###
   
   res_all <- data.frame(Adjust = c(rep("unadjusted", 12), rep("adjusted", 9)),
                         Calculation = c(rownames(res_w), rownames(res_adj)[-(2:3)]),
   ...
   write.csv(res_all, paste0(here_koco_results("R4"), "/r4_cum_sp_w_seed", seed, ".csv"), row.names = FALSE)
